...
def post_process_structured_data(structured_data: dict):
    ...
    if "personal_info" in structured_data:
        ...
        # But we'll let the LLM handle it primarily through the improved prompt
    
    return structured_data

def parse_with_llm(merged_text: str, document_type: str = None):
    """Parse merged text into structured JSON using Mistral 7B."""
    print("\n[LLM PARSING] Parsing text into structured JSON...")
    start_time = time.time()
    ...
    print(f"[LLM PARSING] Detected document type: {document_type}")
    
    # Create prompt based on document type
    if document_type == "driver_license":
        prompt = f"""Parse this driver license text into structured JSON.
...
Extract all visible information. Preserve exact values as they appear in the text. Return ONLY valid JSON, no additional text."""
        ...
        prompt = f"""Parse this insurance card text into structured JSON.
...
Pay attention to table structure. The card may have a table with 'In Ntwk' and 'Out Ntwk' columns. Map values correctly to their columns and rows (Individual vs Family). Extract all visible information. Return ONLY valid JSON, no additional text."""
        ...
        prompt = f"""Parse this document text into structured JSON.
...
Return a JSON object with a logical structure based on the document content. Extract all visible information and organize it into appropriate fields. Return ONLY valid JSON, no additional text."""
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[LLM PARSING] Error: API returned status {response.status_code}")
        ...
        try:
            ...
            print(f"[LLM PARSING] Successfully parsed into structured JSON ({parsing_time:.2f}s)")
            return structured_data, {
                "model": "mistral:7b",
            ...
            print(f"[LLM PARSING] Warning: Failed to parse JSON response: {e}")
            print(f"[LLM PARSING] Raw response: {parsed_text[:500]}...")
        ...
        parsing_time = time.time() - start_time
        print(f"[LLM PARSING] Parsing timed out after 180 seconds")
        return None, {"model": "mistral:7b", "status": "failed", "error": "Timeout", "parsing_time": parsing_time}
    except Exception as e:
        parsing_time = time.time() - start_time
        print(f"[LLM PARSING] Error during parsing: {e}")
        import traceback
        traceback.print_exc()
        return None, {"model": "mistral:7b", "status": "failed", "error": str(e), "parsing_time": parsing_time}
...
def extract_with_dual_models(image_path: str):
    ...
    if not deepseek_text:
        ...
        # Parse with LLM even with single model result
        structured_data, parsing_meta = parse_with_llm(qwen_text)
        return {
            "extraction_timestamp": datetime.now().isoformat(),
            "models_used": {
                "qwen3_vl": qwen_meta,
                "llm_parser": {"model": "mistral:7b"}
            },
            ...
            "metadata": {"total_extraction_time": round(total_time, 2), "image_size": f"{image.size[0]}x{image.size[1]}", "processing_mode": "single_model_with_llm_parsing"}
        }, image
    
    if not qwen_text:
        print("\n[WARNING] Qwen3-VL failed, using DeepSeek-OCR result only")
        # Parse with LLM even with single model result
        structured_data, parsing_meta = parse_with_llm(deepseek_text)
        return {
            "extraction_timestamp": datetime.now().isoformat(),
            "models_used": {
    ...
    # Parse merged text into structured JSON using LLM
    structured_data, parsing_meta = parse_with_llm(merged_text)
    ...
    output = {
        ...
        "metadata": {
            ...
            "processing_mode": "dual_model_with_llm_parsing"
        }
    }
...
def main():
    ...
    print("\n" + "="*80)
    print("[SUCCESS] Dual-model extraction with LLM parsing complete!")
    print("="*80)
